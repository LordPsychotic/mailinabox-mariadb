...
grep -q "127.0.1.1" /etc/hosts && sed -i "s/^127\.0\.1\.1.*/127.0.1.1 $PRIMARY_HOSTNAME/" /etc/hosts || echo "127.0.1.1 $PRIMARY_HOSTNAME" >> /etc/hosts
...
if ! grep -q "^\s*forwarders" /etc/bind/named.conf.options; then
